...
namespace ADL_Client_Tests.Store
{
    ...
    public class Store_Filesystem_Tests : Base_Tests
    {
        ...
        public void List_Files_Recursive()
        {
            ...
            var pages = this.adls_account_client.ListFilesRecursivePaged(AzureDataLakeClient.Store.FsPath.Root, lfo);
        ...
        public void List_Files_Non_Recursive()
        {
            ...
            var pages = this.adls_account_client.ListFilesPaged(AzureDataLakeClient.Store.FsPath.Root, lfo);
            foreach (var page in pages)
            {
        ...
        public void Basic_File_Scenario()
        {
            ...
            Assert.IsTrue( this.adls_account_client.Exists(fname));
            var fi = this.adls_account_client.GetFileStatus(fname);
            Assert.AreEqual(10,fi.Length);

            using (var s = this.adls_account_client.OpenText(fname))
            {
            ...
            Assert.IsFalse(this.adls_account_client.Exists(fname));
        ...
        public void Basic_File_Concatenate_Scenario()
        {
            ...
            this.adls_account_client.Create(fname2, "World", cfo);          
            this.adls_account_client.Concat(new [] { fname1, fname2 },fname3);
            using (var s = this.adls_account_client.OpenText(fname3))
            {
                var content = s.ReadToEnd();
                Assert.AreEqual("HelloWorld", content);
            }
