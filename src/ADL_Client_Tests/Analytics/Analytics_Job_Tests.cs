...
namespace ADL_Client_Tests.Analytics
{
    ...
    public class Analytics_Job_Tests : Base_Tests
    {
        ...
        public void Verify_Default_Top()
        {
            ...
            var jobs = this.adla_account_client.GetJobs(getjobs_options).ToList();
        ...
        public void Verify_Paging_300()
        {
            this.Initialize();
            var getjobs_options = new AzureDataLakeClient.Analytics.GetJobsOptions();
            getjobs_options.Top = AzureDataLakeClient.Analytics.AnalyticsAccountClient.ADLJobPageSize;

            var jobs = this.adla_account_client.GetJobs(getjobs_options).ToList();
            Assert.AreEqual(AzureDataLakeClient.Analytics.AnalyticsAccountClient.ADLJobPageSize, jobs.Count);
        }
        ...
        public void Verify_Paging_400()
        {
            this.Initialize();
            var getjobs_options = new AzureDataLakeClient.Analytics.GetJobsOptions();
            var top = AzureDataLakeClient.Analytics.AnalyticsAccountClient.ADLJobPageSize + (AzureDataLakeClient.Analytics.AnalyticsAccountClient.ADLJobPageSize/2);
        ...
        public void List_Jobs()
        {
            ...
            foreach (var job in this.adla_account_client.GetJobs(getjobs_options))
            {
        ...
        public void Submit_Job_with_Syntax_Error()
        {
            ...
            var ji2 = this.adla_account_client.GetJob(ji.JobId.Value);
