...
namespace AzureDataLakeClient.Analytics
{
    public class AnalyticsManagementClient: ClientBase
    {
        ...
        {
            this._adla_mgmt_rest_client.StorageAccounts.Add(account.ResourceGroup, account.Name, storage_account, parameters);
        }
        ...
        {
            this._adla_mgmt_rest_client.DataLakeStoreAccounts.Add(account.ResourceGroup, account.Name, storage_account, parameters);
        }
        
        public IEnumerable<ADL.Analytics.Models.StorageAccountInfo> ListStorageAccounts(AnalyticsAccountRef account)
        {
            var initial_page = this._adla_mgmt_rest_client.StorageAccounts.ListByAccount(account.ResourceGroup, account.Name);
            foreach (var acc in RESTUtil.EnumItemsInPages(initial_page, p => this._adla_mgmt_rest_client.StorageAccounts.ListByAccountNext(p.NextPageLink)))
            {
                yield return acc;
            }
        }
        ...
        {
            var initial_page = this._adla_mgmt_rest_client.StorageAccounts.ListStorageContainers(account.ResourceGroup, account.Name, storage_account);
            foreach (var acc in RESTUtil.EnumItemsInPages(initial_page, p => this._adla_mgmt_rest_client.StorageAccounts.ListStorageContainersNext(p.NextPageLink)))
            {
                yield return acc;
            }
        }
        ...
        {
            this._adla_mgmt_rest_client.StorageAccounts.Delete(account.ResourceGroup, account.Name, storage_account);
        }

        public void DeleteDataLakeStoreAccount(AnalyticsAccountRef account, string storage_account)
        {
            this._adla_mgmt_rest_client.DataLakeStoreAccounts.Delete(account.ResourceGroup, account.Name, storage_account);
        }
        ...
        {
            var initial_page = this._adla_mgmt_rest_client.StorageAccounts.ListSasTokens(account.ResourceGroup, account.Name, storage_account, container);
            foreach (var acc in RESTUtil.EnumItemsInPages(initial_page, p => this._adla_mgmt_rest_client.StorageAccounts.ListSasTokensNext(p.NextPageLink)))
            {
                yield return acc;
            }
        }
