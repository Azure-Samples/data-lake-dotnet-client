...
namespace AdlClient.Commands
{
    public class FileSystemCommands 
    {
        ...
        public IEnumerable<FsFileStatusPage> ListFilesPaged(FsPath path, FileListingParameters parameters)
        {
            ...
            while (true)
            {
                var result = RestClients.FileSystem.FileSystem.ListFileStatus(uri.Account, uri.Path, parameters.PageSize, after);
        ...
        {
            var uri = this.GetUri(path);
            var result = this.RestClients.FileSystem.FileSystem.Mkdirs(uri.Account, uri.Path);
        }

        public void Delete(FsPath path)
        {
            var uri = this.GetUri(path);
            var result = this.RestClients.FileSystem.FileSystem.Delete(uri.Account, uri.Path);
        }

        public void Delete(FsPath path, bool recursive)
        {
            var uri = this.GetUri(path);
            var result = this.RestClients.FileSystem.FileSystem.Delete(uri.Account, uri.Path, recursive);
        }
        ...
        public void Create(FsPath path, byte[] bytes, FileCreateParameters parameters)
        {
            ...
            var uri = this.GetUri(path);
            RestClients.FileSystem.FileSystem.Create(uri.Account, uri.Path, memstream, parameters.Overwrite);
        ...
        {
            var uri = this.GetUri(path);
            var info = RestClients.FileSystem.FileSystem.GetFileStatus(uri.Account, uri.Path);
        ...
        {
            var uri = this.GetUri(path);
            var acl_result = this.RestClients.FileSystem.FileSystem.GetAclStatus(uri.Account, uri.Path);
        ...
        {
            var uri = this.GetUri(path);
            this.RestClients.FileSystem.FileSystem.ModifyAclEntries(uri.Account, uri.Path, entry.ToString());
        }
        ...
        public void ModifyAclEntries(FsPath path, IEnumerable<FsAclEntry> entries)
        {
            ...
            this.RestClients.FileSystem.FileSystem.ModifyAclEntries(uri.Account, uri.Path, s);
        }
        ...
        public void SetAcl(FsPath path, IEnumerable<FsAclEntry> entries)
        {
            var uri = this.GetUri(path);
            var s = FsAclEntry.EntriesToString(entries);
            this.RestClients.FileSystem.FileSystem.SetAcl(uri.Account, uri.Path, s);
        }

        public void RemoveAcl(FsPath path)
        {
            var uri = this.GetUri(path);
            this.RestClients.FileSystem.FileSystem.RemoveAcl(uri.Account, uri.Path);
        }
        ...
        public void RemoveAclEntries(FsPath path, IEnumerable<FsAclEntry> entries)
        {
            ...
            this.RestClients.FileSystem.FileSystem.RemoveAclEntries(uri.Account, uri.Path, s);
        }

        public void RemoveDefaultAcl(FsPath path)
        {
            var uri = this.GetUri(path);
            this.RestClients.FileSystem.FileSystem.RemoveDefaultAcl(uri.Account, uri.Path);
        }

        public System.IO.Stream Open(FsPath path)
        {
            var uri = this.GetUri(path);
            return this.RestClients.FileSystem.FileSystem.Open(uri.Account, uri.Path);
        }

        public System.IO.StreamReader OpenText(FsPath path)
        {
            var uri = this.GetUri(path);
            var s = this.RestClients.FileSystem.FileSystem.Open(uri.Account, uri.Path);
        ...
        {
            var uri = this.GetUri(path);
            return this.RestClients.FileSystem.FileSystem.Open(uri.Account, uri.Path, bytesToRead, offset);
        }
        ...
        public void Upload(FsLocalPath src_path, FsPath dest_path, FileUploadParameters parameters)
        {
            ...
            this.RestClients.FileSystem.FileSystem.UploadFile(
                dest_uri.Account,
                src_path.ToString(), 
        ...
        public void Download(FsPath src_path, FsLocalPath dest_path, FileDownloadParameters parameters)
        {
            ...
            this.RestClients.FileSystem.FileSystem.DownloadFile(src_uri.Account, src_uri.Path, dest_path.ToString(), parameters.NumThreads, parameters.Resume, parameters.Overwrite);
        }
        ...
        public void Append(FsPath path, string content)
        {
            ...
            {
                this.RestClients.FileSystem.FileSystem.Append(uri.Account, uri.Path, stream);
            }
        }
        ...
        public void Append(FsPath path, byte[] bytes)
        {
            ...
            {
                var uri = this.GetUri(path);
                this.RestClients.FileSystem.FileSystem.Append(this.Account.Name, uri.Path, stream);
            }
        }
        ...
        public void Concat(IEnumerable<FsPath> src_paths, FsPath dest_path)
        {
            ...
            this.RestClients.FileSystem.FileSystem.Concat(this.Account.Name, dest_path.ToString(), src_file_strings);
        ...
        {
            var uri = this.GetUri(path);
            this.RestClients.FileSystem.FileSystem.SetFileExpiry(uri.Account, uri.Path, MSADLS.Models.ExpiryOptionType.NeverExpire, null);
        ...
        public void SetFileExpiryAbsolute(FsPath path, System.DateTimeOffset expiretime)
        {
            ...
            this.RestClients.FileSystem.FileSystem.SetFileExpiry(uri.Account, uri.Path, MSADLS.Models.ExpiryOptionType.Absolute, unix_time);
        }

        public void SetFileExpiryRelativeToNow(FsPath path, System.TimeSpan timespan)
        {
            var uri = this.GetUri(path);
            this.RestClients.FileSystem.FileSystem.SetFileExpiry(uri.Account, uri.Path, MSADLS.Models.ExpiryOptionType.RelativeToNow, (long)timespan.TotalMilliseconds);
        }

        public void SetFileExpiryRelativeToCreationDate(FsPath path, System.TimeSpan timespan)
        {
            var uri = this.GetUri(path);
            this.RestClients.FileSystem.FileSystem.SetFileExpiry(uri.Account, uri.Path, MSADLS.Models.ExpiryOptionType.RelativeToCreationDate, (long)timespan.TotalMilliseconds);
        }

        public MSADLS.Models.ContentSummary GetContentSummary(FsPath path)
        {
            var uri = this.GetUri(path);
            var summary = this.RestClients.FileSystem.FileSystem.GetContentSummary(uri.Account, uri.Path);
        ...
        {
            var uri = this.GetUri(path);
            this.RestClients.FileSystem.FileSystem.SetOwner(uri.Account, uri.Path, user, group);
        }

        public void Move(FsPath src_path, FsPath dest_path)
        {
            this.RestClients.FileSystem.FileSystem.Rename(this.Account.Name, src_path.ToString(), dest_path.ToString());
        }
    }
}