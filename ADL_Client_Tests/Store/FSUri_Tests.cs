...
namespace ADL_Client_Tests.Store
{
    ...
    public class FSUri_Tests : Base_Tests
    {
        ...
        {
            var u0 = new AzureDataLakeClient.Store.FsUri("adl://ACCOUNT.azuredatalakestore.net/users/USER1");
            Assert.AreEqual("account",u0.Account);
            Assert.AreEqual("/users/USER1", u0.Path);

            var u1 = new AzureDataLakeClient.Store.FsUri("adl://ACCOUNT.azuredatalakestore.net/users/");
            Assert.AreEqual("account", u1.Account);
            Assert.AreEqual("/users/", u1.Path);

            var u2 = new AzureDataLakeClient.Store.FsUri("adl://ACCOUNT.azuredatalakestore.net/users");
            Assert.AreEqual("account", u2.Account);
            Assert.AreEqual("/users", u2.Path);

            var u3 = new AzureDataLakeClient.Store.FsUri("adl://ACCOUNT.azuredatalakestore.net/");
            Assert.AreEqual("account", u3.Account);
            Assert.AreEqual("/", u3.Path);

            var u4 = new AzureDataLakeClient.Store.FsUri("adl://ACCOUNT.azuredatalakestore.net");
        ...
        {
            var u0 = new AzureDataLakeClient.Store.FsUri("ACCOUNT",null);
            Assert.AreEqual("account", u0.Account);
            Assert.AreEqual("/", u0.Path);

            var u1 = new AzureDataLakeClient.Store.FsUri("ACCOUNT", "");
            Assert.AreEqual("account", u1.Account);
            Assert.AreEqual("/", u1.Path);

            var u2 = new AzureDataLakeClient.Store.FsUri("ACCOUNT", "\\");
            Assert.AreEqual("account", u2.Account);
            Assert.AreEqual("/", u2.Path);

            var u3 = new AzureDataLakeClient.Store.FsUri("ACCOUNT", "/");
            Assert.AreEqual("account", u3.Account);
            Assert.AreEqual("/", u3.Path);
        }
        ...
        public void FSPath_verify_constructor_normalizes_account_name()
        {
            ...
            var u1 = new AzureDataLakeClient.Store.FsUri("swebhdfs://ACCOUNT.azuredatalakestore.net/users/USER1");
            Assert.AreEqual("adl://account.azuredatalakestore.net/users/USER1", u1.ToUriString());
            Assert.AreEqual("account", u1.Account);

            var u2 = new AzureDataLakeClient.Store.FsUri("webhdfs://ACCOUNT.azuredatalakestore.net/users/USER1");
            ...
            var u5 = new AzureDataLakeClient.Store.FsUri("adl://ACCOUNT.azuredatalakestore.net\\");
